#!/usr/bin/env python3
"""Cooperative fixed-load ECM v6 EMAPT runner."""
...
from experiments.auto_run_v6 import (
    _apply_v6_policies,
    _preflight_cleanup,
    _traffic_profile,
    _write_profile_meta,
...
def main():
    parser = argparse.ArgumentParser(description='Cooperative fixed-load ECM v6 EMAPT runner')
    parser.add_argument('--scenario', choices=['baseline', 'sdnv'], default='sdnv')
    parser.add_argument('--results-tag', default='emapt_v6')
    ...
    meta_path = f'results/emapt_{args.results_tag}_{timestamp}_profile_meta.log'
    ...
    try:
        ...
        info('*** starting helper and local background TCP flows\n')
        bg_processes = []
        for helper in helpers:
            proc, logf = _popen_in_node(
                helper,
        ...
        proc, logf = _popen_in_node(
            sta1,
            f'iperf -c 10.0.0.100 -p {best_effort_port} -t {background_duration} -i 5',
            f'logs/sta1_local_tcp_{args.results_tag}_{timestamp}.log',
        )
        bg_processes.append((proc, logf))
        ...
        policy_log = f'logs/policy_timing_{args.results_tag}_{timestamp}.log'
