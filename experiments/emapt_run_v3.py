#!/usr/bin/env python3
"""Trigger-based v3 EMAPT measurement under common pre-trigger conditions."""
...
def _apply_trigger_policy(sta1, scenario):
    if scenario == 'sdnv':
        info('*** emergency trigger: applying SDNV policy on sta1 for dissemination\n')
        proc, _ = _popen_in_node(sta1, 'bash vehicle/sdnv_policy.sh')
        proc.wait()
    else:
        info('*** emergency trigger: baseline keeps flat policy on sta1 for dissemination\n')


def main():
    parser = argparse.ArgumentParser(description='Trigger-based v3 EMAPT runner')
    parser.add_argument('--scenario', choices=['baseline', 'sdnv'], default='sdnv')
    parser.add_argument('--results-tag', default='emapt_v3')
    ...
    emapt_bg_duration = max(4, int(math.ceil(_float_env('SDNV_EMAPT_BG_DURATION', 10.0))))
    ...
    try:
        ...
        background_duration = int(math.ceil(warmup + emapt_bg_duration + 5))
        ...
        if warmup > 0:
            info('*** starting local TCP warm-up flow on sta1\n')
            warmup_proc, warmup_log = _popen_in_node(
                sta1,
                f'iperf -c 10.0.0.100 -p {best_effort_port} -t {max(1, int(math.ceil(warmup)))} -i 1',
                f'logs/local_warmup_tcp_{args.results_tag}_{timestamp}.log',
            )
        else:
            warmup_proc, warmup_log = None, None
        ...
        _apply_trigger_policy(sta1, args.scenario)

        info('*** starting post-trigger local TCP competitor and emergency dissemination flow from sta1\n')
        local_tcp_proc, local_tcp_log = _popen_in_node(
            sta1,
            f'iperf -c 10.0.0.100 -p {best_effort_port} -t {emapt_bg_duration} -i 1',
            f'logs/local_emapt_tcp_{args.results_tag}_{timestamp}.log',
        )
