#!/usr/bin/env python3
"""
Trigger-based v3 SDNV experiment runner.

Both baseline and SDNV start under the same flat shaping policy on the
emergency vehicle. Background traffic is present during the warm-up period.
At the emergency trigger:
  - baseline keeps the flat policy, so local TCP keeps competing with UDP
  - SDNV switches the emergency vehicle to the prioritized policy, which
    throttles best-effort TCP and protects emergency UDP / dissemination
"""
...
def _apply_policy(sta1, scenario, policy_log):
    if scenario == 'sdnv':
        info('*** emergency trigger: applying SDNV policy on sta1\n')
        ...
        info('*** emergency trigger: baseline keeps flat policy on sta1\n')


def main():
    parser = argparse.ArgumentParser(description='Trigger-based v3 SDNV experiment runner')
    ...
    warmup = args.warmup
    if warmup is None:
        warmup = _float_env('SDNV_WARMUP', 10.0)

    background_total_mbit = _float_env('SDNV_BG_TOTAL_MBIT', 20.0)
    ...
    warmup_tcp_duration = max(1, int(math.ceil(warmup)))
    ...
    try:
        ...
        info('*** applying common pre-trigger flat policy on sta1\n')
        ...
        info('*** starting fixed-load background flows on non-emergency vehicles\n')
        ...
        if warmup > 0:
            info('*** starting local TCP warm-up flow on sta1 under the common baseline policy\n')
            warmup_proc, warmup_log = _popen_in_node(
                sta1,
                f'iperf -c 10.0.0.100 -p {best_effort_port} -t {warmup_tcp_duration} -i 1',
                f'logs/local_warmup_tcp_{results_tag}_{timestamp}.log',
            )
        ...
        _apply_policy(sta1, args.scenario, policy_log)

        info('*** starting concurrent post-trigger emergency/background measurements from sta1\n')
