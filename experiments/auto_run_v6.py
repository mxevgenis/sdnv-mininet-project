#!/usr/bin/env python3
"""
Cooperative fixed-load ECM v6 experiment runner.

Scenario:
  - sta1 is the emergency vehicle
  - every other vehicle becomes a helper when ECM is activated
  - sta1 also runs a local background TCP flow
  - all helpers send cooperative UDP and background TCP after the trigger
  - baseline keeps flat shaping, so local/helper TCP keeps competing with UDP
  - SDNV switches helpers to HTB classes that protect helper UDP and
    aggressively throttle helper TCP, while sta1's local TCP is pushed into
    the low-priority BE class
"""
...
def _traffic_profile(helper_count):
    emergency_rate = _parse_rate_mbit(os.environ.get('EMERGENCY_RATE', '10m'))
    helper_udp_total = _float_env('SDNV_V6_HELPER_UDP_TOTAL_MBIT', 10.0)
    helper_tcp_total = _float_env('SDNV_V6_HELPER_TCP_TOTAL_MBIT', 80.0)
    helper_tcp_limit_total = _float_env('SDNV_V6_HELPER_TCP_LIMIT_TOTAL_MBIT', 10.0)
    helper_udp_floor = _float_env('SDNV_V6_HELPER_MIN_UDP_MBIT', 0.25)
    helper_tcp_floor = _float_env('SDNV_V6_HELPER_MIN_TCP_MBIT', 0.25)
...
def _apply_v6_policies(sta1, helpers, helper_ports, scenario, policy_log, profile):
    if scenario != 'sdnv':
...
def main():
    parser = argparse.ArgumentParser(description='Cooperative fixed-load ECM v6 experiment runner')
    ...
    results_tag = args.results_tag or f'{args.scenario}_v6_v{num_vehicles}'
    ...
    profile = _traffic_profile(helper_count)
    ...
    try:
        ...
        if warmup > 0 and helpers:
            info('*** starting helper and local warm-up TCP flows\n')
            warmup_processes = []
            ...
            proc, logf = _popen_in_node(
                sta1,
                f'iperf -c 10.0.0.100 -p {best_effort_port} -t {warmup_duration} -i 1',
                f'logs/sta1_local_tcp_warmup_{results_tag}_{timestamp}.log',
            )
            warmup_processes.append((proc, logf))
        ...
        _apply_v6_policies(sta1, helpers, helper_udp_ports, args.scenario, policy_log, profile)

        info('*** starting concurrent emergency, local TCP, helper traffic, and latency probes\n')
        run_processes = []
        ...
        proc, logf = _popen_in_node(
            sta1,
            f'iperf -c 10.0.0.100 -p {best_effort_port} '
            f'-t {args.duration} -i 5 | tee {result_dir}/local_tcp_{timestamp}.log',
            f'logs/local_tcp_{results_tag}_{timestamp}.log',
        )
        run_processes.append((proc, logf))
